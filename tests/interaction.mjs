...
try{
 await page.goto(base);await checkHeading(/Vanderbilt/);
 ...
 await page.keyboard.press('ArrowRight');await waitPosition(14);await checkHeading(/Stanford/);
 await page.keyboard.press('ArrowLeft');await waitPosition(13);await checkHeading(/Vanderbilt/);
 await page.getByRole('button',{name:'CT slice 20 of 34',exact:true}).click();await checkHeading(/Emory/);
 assert.equal(await page.locator('.thumbnail[aria-current]').getAttribute('data-slice'),'19');
